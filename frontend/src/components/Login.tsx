			...
			<div className="flex  pt-28 justify-center">
				<div className="flex w-full lg:w-1/2  relative justify-center items-center space-y-8">
					<div className="w-full   px-8 md:px-32 rounded-2xl lg:px-24">
						{otp ? (
							<form
								onSubmit={handleSubmit(handlePhoneNumber)}
								key={"phone"}
								className="bg-white h-[20rem]  rounded-2xl shadow-2xl p-12">
							...
							<form
								key={"otp"}
								onSubmit={data.handleSubmit(hanldleFormOtp)}
								className="bg-white h-[20rem] rounded-2xl shadow-2xl p-12">
								<h1 className="text-gray-800 font-bold text-2xl mb-1">Verify</h1>
								{data.errors.otp?.message ? (
									<span className="text-sm font-normal text-red-600 mb-8">{data.errors.otp?.message}</span>
								) : (
